 ...
 */
 ...
 function generaGriglia(anno: number, mese: number): { giorno: number; corrente: boolean; iso: string }[] {
   ...
   while (celle.length % 7 !== 0) {
     ...
     celle.push({ giorno: ex++, corrente: false, iso: `${annoSuc}-${String(meseSuc+1).padStart(2,"0")}-${String(ex-1).padStart(2,"0")}` });
   }
   return celle;
 }
 ...
 function badgeRichiesta(stato: string) {
   switch (stato) {
     case "confermata": return { label: "Confermata", bg: C.greenLight,  color: C.greenDark };
     case "rifiutata":  return { label: "Rifiutata",  bg: "#FCEBEB",     color: "#A32D2D"   };
     default:           return { label: "In attesa",  bg: C.amberLight,  color: C.amberDark };
   }
 }
